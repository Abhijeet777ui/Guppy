...
## 0.2 Launch gate (Phase 9) — status (2026-08-19)

**Verdict: the real-model leg is already recorded; the container leg is now recorded too (2026-08-20); the suite is re-verified clean.**

- **20-fixture free-tier run — ✅ done (2026-08-16).** `qwen/qwen3.6-27b` on Groq free, **20/20 PASS** (10 bugfix + 5 testadd + 5 refactor), reached by rotating 3 Groq free keys (200k tokens/day each). Authoritative artifact: `docs/bench-results/launch-qwen-groq/merged-results.json`. The sibling `report.md` is the *first-key* truncated run (11/20, most misses are 429 rate-limits) — kept as evidence of the rotation; `merged-results.json` is the final 20/20.
- **Real live `guppy run` — ✅ recorded (2026-08-16).** `docs/live/live-run.md`: bugfix-clamp, baseline red → `apply_patch` → gate green → merge-back (1 file), ~39s, 12.6k tokens.
- **Real `guppy chat` — ✅ recorded (2026-08-16).** `docs/live/live-chat.md`: a tool-backed answer over the full gated loop, gates green, non-git tool errors handled gracefully.
- **Container-mode real run — ✅ recorded (2026-08-20).** `docs/live/live-run-container.md`: bugfix-clamp in the Docker sandbox (`guppy/executor:latest`, no `--local`) — baseline `npm test` red inside the container → `apply_patch` → gate green (13/13, verified in the container) → merge-back (1 file), ~68s / 19.1k tokens / 5 tool calls on `cohere/north-mini-code:free` (OpenRouter free). The live run surfaced a real bug the mock e2e missed: container `exec` without an explicit timeout fired immediately (`setTimeout(fn, undefined)`), so every model `run_command` died with `container exec timed out after undefinedms` (the scripted mock ignores tool errors, so the e2e stayed green). Fixed in `packages/workspace/src/index.ts` — the container exec timeout now defaults to the config timeout, matching `execLocal` — and the recording is the post-fix run. **Follow-up (same day): the container resume e2e is fixed too** — it was failing because `npx --no-install` / `npm run` inside the sandbox drop an EMPTY `node_modules` dir into the worktree, which made `buildBinds` treat "dir exists" as "deps present" and skip the source mount on resume (tsc/eslint unresolvable → gate red). Fixed in `packages/workspace` (empty dirs no longer count as deps; adopt clears the artifact and re-links) with a regression test; run/merge + resume container e2e both green.
- **Re-verified today (2026-08-19):** `guppy-bench sanity` — **21/21 fixtures clean** (clean repo green, mutated red; the suite now includes `longhorizon-ledger`). Build green, **322 tests** across 13 packages.
- **Re-verified (2026-08-20, full suite):** `pnpm -r run test` — **323 tests across 13 packages, all green** (memory 21, event-store 11, workspace 24, sleep-cycle 6, verification-engine 19, context-engine 8, skills 13, core 53, models 43, bench-runner 25, mcp 16, control-plane 84; contracts passes via `--passWithNoTests`). `guppy-bench sanity` — **21/21 fixtures clean** (clean green, mutated red). **The container e2e executed for real this time** (Docker Desktop running): run/merge + resume both pass — the first full-suite green that includes the live container path.
- **Re-verified (2026-08-21, cold-clone):** a fresh `git clone` of `main` followed the README quick start end-to-end — install 59.6s, build 28.5s, suite green — now **325 tests across 13 packages** (verification-engine grew 19 → 21 with the 0.5 level-6 tests; the 08-20 bullet above records the then-accurate 323). Two onboarding warts found and fixed: (1) the process-spawning e2e tests — workspace dependency provisioning (`deps.test.ts`, spawns real `npm`) and core's recursive subagent e2e (`subagent.e2e.test.ts`, spawns child processes) — trip vitest's 5000ms default under parallel load on a cold machine; `@guppy/workspace` and `@guppy/core` now run with `--testTimeout=15000`, matching the control-plane pattern, and both are verified green under a 3-way parallel load; (2) `guppy-bench` / `sleep-cycle` bin shims fail to create at install time because `dist/` doesn't exist pre-build (pnpm doesn't retro-create them after `pnpm build`) — documented in `CONTRIBUTING.md`, the README's quick start avoids them by design.
- **Roadmap 0.4 — TUI visual sign-off DONE (2026-08-21).** `docs/live/tui-signoff.md`: headless screen dumps (5 checkpoints: boot, chat turn, /plan, plan produced, /build) plus a **real-model Groq TUI session** (`groq/qwen/qwen3.6-27b`, key from `~/.guppy/config.json`) driving `runTui` headlessly against a red clamp fixture — the real turn landed `success` (2 tool calls, 5k tokens, ~11s, gate escalated to level 3 and passed). The reproducible driver is `apps/control-plane/.scratch/tui-signoff.mjs`. **User signed off the look — M1/M3 closed.**
- **Roadmap 0.5 — level 6 decided (ADR-013, 2026-08-21).** The ladder top is **redefined as the repo-declared invariant gate**: `-v 6` is accepted everywhere (run/chat/TUI `/verify`); a repo opts in via `guppy.json` `verification.levels.6` (dafny, a custom checker, …); when no invariant tool is configured the level is a skip-with-note (ADR-011), never a failure. Dafny stays in the deferred-forever locked decisions.
- **Roadmap 0.6 — compression real numbers DONE (2026-08-21).** `docs/live/live-compression-ab.md`: real-model A/B on the `longhorizon-ledger` fixture — baseline (no compression) vs **deterministic recap** vs **LLM summary**, same 1000-token budget, all **success** on `qwen/qwen3.6-27b` (Groq free). Findings: LLM summary is net-negative for a 1-turn span (+2,100 summary tokens vs −4 for deterministic); the mechanism pays off long-horizon — a full 47 KB-ledger run on `cohere/north-mini-code:free` (OpenRouter free) compressed **16×** (avg −4,792 est tokens each, history bounded at ~25k) before dying to OpenRouter's free daily cap (50 req/day). The quota wall is now documented with numbers (Groq free 8k TPM caps payloads; OpenRouter free caps at 50 req/day), not just asserted.
- **Blocker for a *fresh* re-run:** the free-tier keys in `.env` are currently empty placeholders, and the shell's `GOOGLE_API_KEY` is rejected by Gemini ("Please pass a valid API key"), so a new 20-fixture run needs a working Groq/OpenRouter/Gemini key dropped into `.env` (or the shell env).
...
- 7 levels: `0 syntax, 1 typecheck, 2 lint, 3 unit-tests, 4 property-tests, 5 integration-tests, 6 invariant-gate` (ADR-013: level 6 is the repo-declared invariant gate — `-v 6` is accepted, and when a repo has no invariant tool configured the level is a skip-with-note, never a failure).
...
### 3.8 Bench runner
- 2 configs: `guppy-core` (native loop) and `guppy-core-skill` (skills injected from `--skills`); the prime/pi baseline configs were removed as dead weight.
- `--dry-run` (materialize + gate without any LLM), `sanity` (every fixture clean→green, mutated→red), `--max-attempts`, `--attempt-timeout`, per-config model/provider/base-url/api-key, `loop-demo`, `skill-demo`, `sleep-cycle`.
- **Loud model failures:** a 0-token failure trajectory that carries an error is surfaced as an infrastructure failure (model unreachable / bad key), not a silent agent outcome (see §7.3).
...
| **Verification levels** | Levels 0-5 exercised end-to-end (1 tsc, 2 lint via real eslint 9 + hermetic shim, 3 `npm test`, 4 property, 5 integration); LintPassed/Failed events in the store; **level 6 accepted as the repo-declared invariant gate (ADR-013)** — `-v 6` works, skipped-with-note when no invariant tool is configured | Real-invariant runs depend on the repo declaring `verification.levels.6` in guppy.json (dafny or a custom checker); real-eslint run depends on the repo having eslint installed | The ladder top is now defined instead of rejected; gate fidelity at the top proven |
...
`guppy chat` opens an interactive session over the same SessionManager loop — a **fullscreen TUI** on a TTY (scrollable transcript + input dock + status line + `/models` picker) with the readline REPL as the non-TTY fallback. Each message is a gated task run, streamed live, with per-turn summaries, `/help`, `/verify <0-6>` (6 = repo-declared invariant gate, skipped when unconfigured), and `/exit`.
