...
- **Tests:** `pnpm -r run test` (root `pnpm test`) — green end-to-end; 135 tests across 10 suites (only `contracts` passes via `--passWithNoTests`):
  ...
  | Suite | Tests | Covers |
  |---|---|---|
  | `@guppy/core` | 26 | OpenAI client (request shape, auth, errors, fenced-JSON + `<function/name>` text-tool-call fallbacks, assistant `tool_calls.type` normalization, streaming SSE, backoff, per-request timeout), full core tool loop E2E, rich tools (search/apply_patch/git) |
...
| **Rate limiting** | Retry/backoff shipped (429/5xx/network, Retry-After aware, CLI-tunable); hit the free-tier cap for real | Backoff under sustained 429 on the 50 req/day free tier is untested | Unattended free-tier runs may still exhaust the daily cap |
